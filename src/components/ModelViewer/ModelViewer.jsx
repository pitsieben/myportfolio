  ...
  return (
    <Html center>
      {placeholderSrc ? (
        ...
        <span style={{ color: '#ffffff', fontSize: '1.2rem', fontFamily: 'sans-serif' }}>
          {Math.round(progress)} %
        </span>
      )}
    </Html>
...
const ObjectLoader = ({ url, onLoaded }) => {
  const obj = useLoader(OBJLoader, url);
  useEffect(() => {
    if (obj) onLoaded(obj.clone());
  }, [obj, onLoaded]);
  return null;
};

const GLTFLoaderWrapper = ({ url, onLoaded }) => {
  const gltf = useGLTF(url);
  useEffect(() => {
    if (gltf?.scene) onLoaded(gltf.scene.clone());
  }, [gltf, onLoaded]);
  return null;
};

const FBXLoaderWrapper = ({ url, onLoaded }) => {
  const fbx = useFBX(url);
  useEffect(() => {
    if (fbx) onLoaded(fbx.clone());
  }, [fbx, onLoaded]);
  return null;
};
...
const ModelInner = ({
  ...
  useLayoutEffect(() => {
    ...
    // Translate the content so its center is at the origin
    ...
    g.traverse(o => {
      if (o.isMesh) {
        ...
        // Ensure sleek robotic material appearance if OBJ default material is plain
        if (ext === 'obj' && (!o.material || o.material.isMeshBasicMaterial || o.material.isMeshPhongMaterial)) {
          o.material = new THREE.MeshStandardMaterial({
            color: o.material?.color || 0xd8d8d8,
            metalness: 0.7,
            roughness: 0.25
          });
        }
  ...
  return (
    <>
      {!content && (
        ext === 'obj' ? <ObjectLoader url={url} onLoaded={setContent} /> :
        ext === 'fbx' ? <FBXLoaderWrapper url={url} onLoaded={setContent} /> :
        <GLTFLoaderWrapper url={url} onLoaded={setContent} />
      )}
      {content && (
        <group ref={outer}>
          <group ref={inner}>
